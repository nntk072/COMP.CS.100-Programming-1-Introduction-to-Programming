...
def main():
    ...
    while True:
        ...
        if "example".startswith(command) and parameters != "":
            ...
            u1 = []  # Variable to get u2, don't need to worry about this variable
            # u2 = [] # Variable to get the beginning and the ending of the data
            variable = 0
            ...
            """for i in range(0, len(u1), 2): u2.append([u1[i], u1[i + 1]])  # After finishing this step, we can get 
            the beginning and the end to the list of a list print(u2) """
            for i in range(0, len(u1), 2):  # variable to get the beginning and the ending of the data
                for z in range(u1[i], u1[i + 1] + 1, 1):  # we can get the line number to get the data with t
                    ...
                    # print(filelist[t].split())
                    # u = filelist[t].split()  # list the word of the list into small list
                    # Remove the comment of the list
                    ...
                    if len(finallist) < (u1[i + 1] - u1[i] + 1):
                        finallist.append(u)

                    else:
                        dictionary[f"Data {variable}"] = finallist
                        finallist = []
                        variable += 1
